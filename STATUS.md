...
- **Page-scoped rules** — `.peoplewrapper` / `.peoplephoto` / `.photo-gallery` / `.publist` / `.paper-title` / `.science-row` (the Science page's alternating blurb-plus-Bluesky-card rows) all live in `_sass/_08_pages.scss`. No `<style>` blocks remain in `pages/*.md`.
